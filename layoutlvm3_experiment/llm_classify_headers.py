...
def save_metrics_to_csv(metrics: Dict[str, Any], output_file: str, method_name: str, model_name: str = None) -> None:
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as f:
        ...
        # Header
        writer.writerow([
            'method', 'model', 'precision', 'recall', 'f1_score', 
...
def main():
    ...
    parser.add_argument("--output_dir", default="llm_results", help="Directory to save detailed results")
    ...
    method_name = f"ollama_{args.model.replace(':', '_')}"
    ...
    if args.gold:
        ...
        if args.metrics_csv:
            save_metrics_to_csv(metrics, args.metrics_csv, method_name, args.model)
        else:
            # Default metrics CSV filename
            metrics_file = Path(args.output_dir) / f"{method_name}_metrics.csv"
            save_metrics_to_csv(metrics, str(metrics_file), method_name, args.model)
