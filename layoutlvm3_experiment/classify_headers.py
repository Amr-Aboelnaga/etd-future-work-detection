...
def lexical_classify(text: str, fuzzy_thresh: float = 80.0, use_partial_matching: bool = False) -> tuple:
...
def reclassify_headers(pdf_results: List[Dict[str, Any]], 
                      ...
                      verbose: bool = False,
                      num_gpus: int = 6,
                      use_partial_matching: bool = False) -> List[Dict[str, Any]]:
    ...
    for pdf_data in tqdm(pdf_results, desc=f"Classifying with {method}"):
        ...
        for header in pdf_data["all_predictions"]:
            ...
            if method == "lexical":
                label, score = lexical_classify(text, use_partial_matching=use_partial_matching)
...
def evaluate_predictions(results: List[Dict[str, Any]], gold_csv: Path) -> Dict[str, Any]:
    ...
    for result in results:
        ...
        for entry in gold_entries:
            ...
            label = entry['label']
            if any(target_label in label for target_label in target_labels):
...
def main():
    ...
    parser.add_argument("--output", help="Output JSON file with reclassified results")
